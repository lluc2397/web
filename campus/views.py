...
from perfil.models import Follow, Perfil
from perfil.forms import UserUpdateForm, ProfileUpdateForm
import sys
...
def SearchView(request):
    if request.method == 'POST':
        ...
        print(kerko)
        results = MyUser.objects.filter(username__contains=kerko)
        context = {
            'results':results
        }
...
class UserPostListView(LoginRequiredMixin, ListView):
    ...
    def visible_user(self):
        return get_object_or_404(MyUser, username=self.kwargs.get('username'))
...
class PostCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        data = super().get_context_data(**kwargs)
        data['tag_line'] = 'Escribe tu pregunta'
        return data
...
class PostUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        data = super().get_context_data(**kwargs)
        data['tag_line'] = 'Modifica tu pregunta'
        return data
